Feature: Suspend a subscription
  ...
  Background:
    Given an admin: "Homer" exists
      ...
      And a subscription exists with publication: publication "p01", user: user "u01", state: "active"
      And a subscription exists with publication: publication "p02", user: user "u02", state: "suspended"
  ...
  Scenario: An admin can suspend a subscription
    When I follow "Suspend"
     And I fill in "Number of days to suspend subscription" with "30"
     And I press "Suspend"
    Then I should see "Subscription to publication 01 for f01 l01 suspended for 30 days"
