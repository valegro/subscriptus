...
describe Wordpress do
  ...
  context "#create" do
    ...
    it "should return the newly created user's login" do
      stub_request(:get, Wordpress.config[:endpoint]).with(:query => {:func => 'create', :email => 'joebloggs@example.com', :login => 'joebloggs', :pword => 'password'}).to_return(:body => 'joebloggs')
    ...
    it "should raise an error if the user already exists" do
      stub_request(:get, Wordpress.config[:endpoint]).with(:query => {:func => 'create', :email => 'joebloggs@example.com', :login => 'joebloggs', :pword => 'password'}).to_return(:body => 'User already exists')
  ...
  context "#update" do
    ...
    it "should return the updated user's login" do
      stub_request(:get, Wordpress.config[:endpoint]).with(:query => {:login => 'joebloggs', :func => 'update', :firstname => 'Joe'}).to_return(:body => 'joebloggs')
      Wordpress.update(:login => 'joebloggs', :firstname => 'Joe').should == 'joebloggs'
    end
    
    it "should raise an error if the user does not exist" do
      stub_request(:get, Wordpress.config[:endpoint]).with(:query => {:login => 'janebloggs', :func => 'update', :firstname => 'Jane'}).to_return(:body => 'User does not exists!')
