...
describe SubscribeController do
  ...
  describe "on update" do
    describe "if no offer is set" do
      before(:each) do
        ...
        @subscription = Factory.create(:active_subscription, :user => @user)
      ...
      it "should use the publication's default for renewal if one is set" do
        # PUT data
        get(:edit)
        @subscription.actions.last.term_length.should == @termd.months
      end

      it "should use the first offer for the publication otherwise" do
        @subscription = Factory.create(:active_subscription, :publication => @publication)
        ...
        @subscription.actions.last.term_length.should == @terma.months
      end
    end
  end
end
